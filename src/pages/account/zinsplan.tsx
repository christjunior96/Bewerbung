import Head from 'next/head'
import Image from 'next/image'
import { GetServerSideProps, InferGetServerSidePropsType } from "next";
import Headerbar from 'components/Organism/account/AccountWrapper';
import Section from 'components/Molecule/Section';
import AccountWrapper from 'components/Organism/account/AccountWrapper';
import InterestPlan from 'components/Organism/account/InterestPlan';



export default function Home(props: InferGetServerSidePropsType<typeof getServerSideProps>) {


    return (
        <>
        <Head>
            <meta name="viewport" content="width=device-width, initial-scale=1" />
            <link rel="icon" href="/favicon.ico" />
        </Head>
        <main>
            <Section>
                <AccountWrapper headline='Zins & Tilgungsplan' selected='item3' name='Herr Diaz Rodriguez' bildUrl='/metzger.jpg' letzterLogin='27.02.2023, 07:45'>
                    <InterestPlan interest={5} investDate={new Date(2023, 1, 23)}  endDate={new Date(2030, 1,5)} investSum={5000} projectName='Windpark Bad Nauheim'></InterestPlan>
                    <InterestPlan interest={5} investDate={new Date(2023, 1, 23)}  endDate={new Date(2030, 1,5)} investSum={5000} projectName='Windpark Bad Nauheim'></InterestPlan>
                </AccountWrapper>
            </Section>
        </main>
        </>
    )
    }

export const getServerSideProps: GetServerSideProps = async (context) => {
// Holt sich die Cookies aus dem request

    return { props: {} };
}
